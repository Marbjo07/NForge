...
TensorLayout::TensorLayout(const Tensor::Shape& _shape) : rank(_shape.getNumDims()) {
...
TensorLayout::TensorLayout(std::array<size_t, MAX_DIMS> _shape,
                           std::array<size_t, MAX_DIMS> _strides, size_t _offset, size_t _rank)
...
bool TensorLayout::operator==(const TensorLayout& rhs) const {
	...
	for (int d = rank - 1; d >= 0; d--) {
		if (shape[d] != rhs.shape[d])
			return false;
		if (strides[d] != rhs.strides[d])
			return false;
	}
...
bool TensorLayout::operator!=(const TensorLayout& rhs) const { return !operator==(rhs); }