...
Tensor::View::View(Tensor& parent, const std::vector<size_t>& stride, const Tensor::Shape& shape,
                   BroadcastTag)
    : m_parent(parent), m_layout(shape, stride), m_position({}) {}

Tensor::View::View(Tensor& parent, const std::vector<size_t>& position, const TensorLayout& layout)
...
Tensor::View Tensor::View::broadcast(Tensor& source, const Tensor::Shape& targetShape) {
	...
	for (size_t i = 0; i < srcShape.getNumDims(); i++) {
		...
		if (srcDim == tgtDim) {
			...
			throw std::runtime_error("Can't broadcast shape " + srcShape.toString() + " to " +
			                         targetShape.toString() + ": dimension " + std::to_string(i) +
			                         " is " + std::to_string(srcDim) + " but target is " +
			                         std::to_string(tgtDim));
		}
	}
...
Tensor::View Tensor::View::subsample(const Tensor::View& src, const std::vector<size_t>& factors) {
	if (src.getShape().getNumDims() != factors.size()) {
		throw std::runtime_error("Can't subsample view of shape" + src.getShape().toString() +
		                         " with factors of rank " + std::to_string(factors.size()));
	}
	...
	if (strides.size() != rank) {
		if (strides.size() == 1 && strides[0] == 0) {
			...
			throw std::runtime_error(
			    "Can't subsample view with different rank strides than shape.");
		}
	}
